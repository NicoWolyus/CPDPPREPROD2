...
function AddImageProduct($ImageXML)
{
    ...
    /* Fixe le context par défaut pour l'image */
                ...
                if (ImageManager::resize($srcfile, $path.'.jpg')) {
                    ...
                    if ($ImageXML->shops) {
                        ...
                        foreach ($ImageXML->shops->shop as $shop) {
                            ...
                            if ($id_shop) {
                                Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'image_shop (`id_product`,`id_image`, `id_shop`, `cover`)
																   VALUES('.(int)($id_product).', '.(int)($image->id).', '.(int)($id_shop).', '.pSQL($cover).')');
                            }
                        }
                    }
                } else {
