  ...
  $('#ls-slider-filters').on('change', 'select', function() { this.form.submit() });
	...
	if (window.lsSliderData) { // EDIT VIEW
		...
		$.getJSON(ajaxurl, {action: 'ls_get_shop_params'}, function(params) {
			...
			lsSliderData.properties.groups = typeof lsSliderData.properties.groups == 'string' ? JSON.parse(lsSliderData.properties.groups) : lsSliderData.properties.groups || [0];
			$groups = createParamSelect(params.groups, 'id_group').attr('multiple', true).val(lsSliderData.properties.groups).css('height', '100%');
			createParamTr(params.groups, $groups).prependTo('.ls-settings-contents tbody:last');
