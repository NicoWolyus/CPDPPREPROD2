...
class Ps_Customtexttop extends Module implements WidgetInterface
{
    ...
    {
        $this->name = 'ps_customtexttop';
        $this->author = 'Be On The Best Side';
        $this->version = '4.1.0';
        ...
        $this->templateFile = 'module:ps_customtexttop/ps_customtext.tpl';
    }
    ...
    public function installDB()
    {
        ...
        $return &= Db::getInstance()->execute('
                CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'infotop` (
        ...
        $return &= Db::getInstance()->execute('
                CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'info_shoptop` (
        ...
        $return &= Db::getInstance()->execute('
                CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'info_langtop` (
    ...
    public function uninstallDB($drop_table = true)
    {
        $ret = true;
        if ($drop_table) {
            $ret &= Db::getInstance()->execute('DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'infotop`')
                && Db::getInstance()->execute('DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'info_shoptop`')
                && Db::getInstance()->execute('DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'info_langtop`');
        }
    ...
    {
        $sql = 'SELECT * FROM `'._DB_PREFIX_.'info_langtop`
