...
class MigrateData
{
    ...
    public function retrieveOldData()
    {
        ...
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'infotop` i
            INNER JOIN `' . _DB_PREFIX_ . 'info_langtop` il ON il.`id_info` = i.`id_info`'
        );
