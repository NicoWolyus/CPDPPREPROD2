...
require_once _PS_MODULE_DIR_ . 'bestkit_productfeatures2/includer.php';

class bestkit_productfeatures2 extends Module
{
    const PREFIX = 'bestkit_pfeatures2_';
    ...
    {
        $this->name = 'bestkit_productfeatures2';
        $this->tab = 'front_office_features2';
        $this->version = '1.7.5';
        $this->author = 'best-kit2';
...
//$this->registerHook('displayProductAdditionalInfo');
        ...
        $this->displayName = $this->l('Product Features2');
    ...
    public function install()
    {
        if (parent::install()) {
            ...
            $sql[] = 'CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'bestkit_pfeatures2` (
            ...
            $sql[] = 'CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'bestkit_pfeatures_color2` (
    ...
    {
        $sql = array();
        $sql[] = 'DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'bestkit_pfeatures`';
        $sql[] = 'DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'bestkit_pfeatures_color`';
        foreach ($sql as $_sql) {
            Db::getInstance()->Execute($_sql);
        }
    ...
    private function printConfigForm() {
        $this->context->smarty->assign(array(
            'bestkit_pfeatures_submit' => $this->context->link->getAdminLink('AdminModules') . '&configure=bestkit_productfeatures2',
            'features' => Feature::getFeatures($this->context->language->id),
        ));

        return $this->context->smarty->fetch(_PS_MODULE_DIR_ . '/bestkit_productfeatures2/views/templates/admin/configForm.tpl');
    }
    ...
    public function getPFeatures()
    {
        return Db::getInstance()->ExecuteS('
                    SELECT pf.*, fl.`name`
                    FROM `'._DB_PREFIX_.'bestkit_pfeatures2` pf
    ...
    public function getPFeatureColors()
    {
        return Db::getInstance()->ExecuteS('
                    ...
                    FROM `'._DB_PREFIX_.'bestkit_pfeatures_color2` pfc
    ...
    public function getFeatureValuesWithLang($id_lang, $id_feature, $custom = false)
    {
        return Db::getInstance()->ExecuteS('
            ...
            LEFT JOIN `'._DB_PREFIX_.'bestkit_pfeatures_color` pfc ON (pfc.`id_feature_value` = v.`id_feature_value`)
...
print_r($pfeatures); die;*/
    ...
    protected function postProcess() {
        if (Tools::isSubmit('submitUpdate') || Tools::isSubmit('submitUpdateAndStay')) {
            if (Tools::isSubmit('main_feature')) {
                Configuration::updateValue(self::PREFIX . 'main_feature', Tools::getValue('main_feature'));
            }

            if (Tools::isSubmit('new_feature')) {
                $new_feature = Tools::getValue('new_feature');
        ...
        if (Tools::isSubmit('delete_pfeature')) {
            ...
            Db::getInstance()->Execute('DELETE FROM `'._DB_PREFIX_.'bestkit_pfeatures2` WHERE id_feature = ' . (int)$delete_pfeature);
        }
        ...
        $this->context->smarty->assign(array(
            'main_feature' => Configuration::get(self::PREFIX . 'main_feature'),
        ));
    }
    ...
    public function getPFeaturesByProduct($id_product)
    {
        ...
        $pfeatures = Db::getInstance()->ExecuteS('
            ...
            join `'._DB_PREFIX_.'bestkit_pfeatures` pf ON (fp.`id_feature` = pf.`id_feature`)
    ...
    public function getProducts($id_product)
    {
        ...
        $products = Db::getInstance()->ExecuteS('
            ...
            left join `'._DB_PREFIX_.'bestkit_pfeatures_color2` pfc ON (pfc.`id_feature_value` = fp.`id_feature_value`)
            ...
            join (
                ...
                where fp.`id_feature_value` = (
                    ...
                    where id_feature = '.(int)Configuration::get(self::PREFIX . 'main_feature').'
...
*/
    ...
    {
        $this->context->smarty->assign('bestkit_pfeatures', array(
    ...
    public function ajaxProcessProductfeaturesPagination()
    {
        ...
        $html = '';
        $html .= $this->context->smarty->fetch(_PS_MODULE_DIR_ . '/bestkit_productfeatures2/views/templates/admin/pfeatures.tpl');
        $html .= $this->context->smarty->fetch(_PS_MODULE_DIR_ . '/bestkit_productfeatures2/views/templates/admin/list_footer.tpl');
//print_r($html); die;
